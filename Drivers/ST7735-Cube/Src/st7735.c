...
void ST7735_RegData(void);
void ST7735_RegCommand(void);
void ST7735_WriteCommand(uint8_t cmd);
void ST7735_WriteData(const uint8_t *buff, size_t buff_size);
...
void flushDisplay()
{
	if (frameBuffer != NULL)
	{
		...
		ST7735_RegData();
		HAL_SPI_Transmit(spiPort, (const uint8_t *)frameBuffer, 2 * _width * _height, HAL_MAX_DELAY);
		ST7735_DeSelect();
	}
}
...
void ST7735_SendCommand(uint8_t commandByte, const uint8_t *dataBytes,
